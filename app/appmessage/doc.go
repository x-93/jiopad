...
supported karlsen messages to and from the appmessage. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# Karlsen Message Overview

The karlsen protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which karlsen network it is a part of, its type, how big it is, and a checksum
to verify validity. All encoding and decoding of message headers is handled by
this package.

To accomplish this, there is a generic interface for karlsen messages named
...
of the currently supported karlsen messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using karlsen encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the karlsen messages are intended
...
and write karlsen messages. The following sections provide a quick overview of
...
The karlsen network is a magic number which is used to identify the start of a
message and which karlsen network the message applies to. This package provides
...
As discussed in the karlsen message overview section, this package reads
and writes karlsen messages using a generic interface named Message. In
...
In order to unmarshall karlsen messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a karlsen peer. Example syntax is:
	...
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, karlsenNet)
	if err != nil {
...
In order to marshall karlsen messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a karlsen peer. Example syntax to request addresses
from a remote peer is:
	...
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, karlsenNet)
	if err != nil {
		// Log and handle the error
	}
