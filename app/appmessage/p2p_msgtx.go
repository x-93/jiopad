...
// Outpoint defines a karlsen data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new karlsen transaction outpoint point with the
...
// TxIn defines a karlsen transaction input.
type TxIn struct {
...
// NewTxIn returns a new karlsen transaction input with the provided
...
// TxOut defines a karlsen transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new karlsen transaction output with the provided
...
// MsgTx implements the Message interface and represents a karlsen tx message.
