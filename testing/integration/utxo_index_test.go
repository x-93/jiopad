...
func TestUTXOIndex(t *testing.T) {
	...
	rewardsMinedSompi := uint64(blockAmountToMine * constants.SompiPerKarlsen * 50)
	...
	rewardsMinedViaBlockCountSompi := uint64(
		(getBlockCountResponse.BlockCount - 2) * constants.SompiPerKarlsen * 50, // -2 because of genesis and virtual.
	)
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = karlsend.rpcClient.SubmitTransaction(rpcTransaction, transactionID, false)
		if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) (*appmessage.RPCTransaction, string) {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixKarlsenSim)
	if err != nil {
